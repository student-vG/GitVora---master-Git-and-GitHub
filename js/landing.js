...
const landingThemeBtnMobile = document.getElementById('themeToggleLandingMobile');

function applyTheme(theme) {
  document.documentElement.setAttribute('data-theme', theme);
  const isLight = theme === 'light';
  if (landingThemeBtn) landingThemeBtn.textContent = isLight ? '🌙 Dark' : '☀️ Light';
  if (landingThemeBtnMobile) landingThemeBtnMobile.textContent = isLight ? '🌙 Dark Mode' : '☀️ Light Mode';
}
...
if (landingThemeBtnMobile) landingThemeBtnMobile.addEventListener('click', toggleTheme);
...
const terminalLines = [
  ...
  { type: 'success', text: '✓ Future pushes work without retyping origin/main' },
...
function typeNextLine() {
  ...
  const cls = line.type === 'prompt' ? 't-prompt' : line.type === 'success' ? 't-success' : 't-info';
  span.className = cls;
  ...
  basics: [
    {
      ...
      detail: 'Creates a new .git directory in your current folder. This is always the first step when starting a new project.',
      example: '$ mkdir my-project\n$ cd my-project\n$ git init\n→ Initialized empty Git repository in .git/',
    },
    {
      cmd: 'git add <file>',
      desc: 'Stage files for commit',
      detail: 'Adds file changes to the staging area. Use "git add ." to stage all changes at once.',
      example: '$ git add index.html        # stage one file\n$ git add .                 # stage everything\n$ git add src/              # stage a folder',
    },
    {
      cmd: 'git commit -m "message"',
      desc: 'Save staged changes with a message',
      detail: 'Creates a snapshot of your staged changes. Write clear, descriptive messages — your future self will thank you.',
      example: '$ git commit -m "feat: add user login form"\n→ [main 4a2bc13] feat: add user login form\n  2 files changed, 54 insertions(+)',
    },
    {
      cmd: 'git status',
      desc: 'Show working tree status',
      detail: 'Shows which files are staged, modified, or untracked. Run this whenever you are unsure of what\'s happening.',
      example: '$ git status\nOn branch main\nChanges to be committed:\n  modified:   index.html\nUntracked files:\n  styles.css',
    },
    {
      cmd: 'git log',
      desc: 'Show commit history',
      detail: 'Lists all commits in the current branch. Use --oneline for a compact view.',
  ...
  branch: [
    {
      ...
      detail: 'Creates a new branch without switching to it. Branches let you work on features without affecting main.',
      example: '$ git branch feature/login\n$ git branch\n  feature/login\n* main',
    },
    {
      cmd: 'git checkout -b <name>',
      desc: 'Create and switch to new branch',
      detail: 'Shortcut to create a branch and immediately switch to it. The most common way to start new work.',
    ...
    },
    {
      ...
      detail: 'Deletes a merged branch. Use -D (capital) to force-delete an unmerged branch.',
  ...
  remote: [
    {
      ...
      detail: 'Links your local repository to a remote one (like GitHub). "origin" is just a conventional name.',
      example: '$ git remote add origin https://github.com/you/repo.git\n$ git remote -v\norigin  https://github.com/you/repo.git (fetch)',
    },
    {
      cmd: 'git push origin main',
      desc: 'Push commits to GitHub',
      detail: 'Uploads your local commits to the remote repository. On first push, use --set-upstream or -u, then future pushes can be plain "git push".',
      example: '$ git push -u origin main\n→ Branch \'main\' set up to track origin/main.\n$ git push\n✓ Push successful',
    },
    {
      cmd: 'git push --force-with-lease',
      desc: 'Safely force-push after rebase',
      detail: 'Use this only when you rewrote your own branch history (for example after rebase). It is safer than --force because it refuses to overwrite teammates\' new commits.',
      example: '$ git pull --rebase origin main\n$ git push --force-with-lease origin feature/my-branch',
    },
    {
      cmd: 'git pull',
      desc: 'Download and integrate changes',
      detail: 'Fetches latest changes from remote and merges them into your local branch. Do this before starting work.',
      example: '$ git pull\n→ Already up to date.\n# or\n→ Updating abc..def: Fast-forward',
    },
    {
      cmd: 'git clone <url>',
      desc: 'Download a repository',
      detail: 'Creates a local copy of a remote repository, including all history.',
  ...
  undo: [
    {
      ...
      detail: 'Reverts a file to the last committed version. Caution: changes are lost permanently.',
    ...
    },
    {
      ...
      example: '$ git add .           # Oops, staged too much\n$ git reset HEAD styles.css\n→ Unstaged changes from styles.css',
    },
    {
      cmd: 'git revert <commit>',
      desc: 'Undo a commit safely',
      detail: 'Creates a new commit that reverses the specified commit. Safe to use on pushed code.',
      example: '$ git revert abc1234\n→ [main def5678] Revert "bad change"',
    },
    {
      cmd: 'git stash',
      desc: 'Temporarily save uncommitted changes',
      detail: 'Saves your dirty working directory so you can switch branches and come back later.',
...
const errorsData = [
  {
    ...
    cause: 'The remote has commits your local branch doesn\'t have. This usually happens when someone else pushed, or when GitHub created a README on init.',
    fix: '$ git pull --rebase origin main\n$ git push origin main',
  },
  {
    tag: 'MERGE CONFLICT',
    title: 'CONFLICT (content): Merge conflict in <file>',
    cause: 'Two branches made different changes to the same lines. Git doesn\'t know which version to keep.',
  ...
  },
  {
    ...
    cause: 'Your local repo and remote repo have no common commits (different initial commits).',
    fix: '$ git pull origin main --allow-unrelated-histories\n$ git push origin main',
  },
  {
    tag: 'AUTH FAILED',
    title: 'remote: Support for password authentication was removed',
    cause: 'GitHub stopped accepting passwords in 2021. You need a Personal Access Token or SSH key.',
  ...
  },
  {
    ...
    cause: 'Your branch has never been pushed. Git doesn\'t know where to push it.',
    fix: '$ git push --set-upstream origin <branch-name>\n# Or shorthand:\n$ git push -u origin <branch-name>\n# Future pushes:\n$ git push',
  },
  {
    tag: 'DETACHED HEAD',
    title: 'HEAD detached at <commit>',
    cause: 'You checked out a specific commit instead of a branch. Commits made here won\'t be on any branch.',
    fix: '# To go back to main safely:\n$ git checkout main\n\n# To keep your changes:\n$ git checkout -b my-new-branch',
  },
  {
    tag: 'NOTHING TO COMMIT',
    title: 'nothing to commit, working tree clean',
    cause: 'Not really an error! It means all your changes are already committed, or you haven\'t made any changes since the last commit.',
  ...
  },
  {
    ...
    cause: 'SSH authentication failed. Either your SSH key isn\'t added to GitHub, or you\'re not using the right key.',
...
function renderErrors(filter = '') {
  ...
  const filtered = filter
    ? errorsData.filter(e => e.title.toLowerCase().includes(filter.toLowerCase()) || e.tag.toLowerCase().includes(filter.toLowerCase()))
    : errorsData;
  ...
  filtered.forEach(e => {
    ...
    `;
    el.querySelector('.error-header').addEventListener('click', () => el.classList.toggle('open'));
...
window.addEventListener('scroll', () => {
  document.querySelector('.navbar').style.borderBottomColor = window.scrollY > 50 ? 'rgba(0,255,136,0.1)' : '';
});
